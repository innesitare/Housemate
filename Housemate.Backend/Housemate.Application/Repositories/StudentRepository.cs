...
public sealed class StudentRepository : IStudentRepository
{
    ...
    public async Task<bool> CreateAsync(Student housingTask, CancellationToken cancellationToken = default)
    {
        await _dbContext.Students.AddAsync(housingTask, cancellationToken);
        int result = await _dbContext.SaveChangesAsync(cancellationToken);

        return result > 0;
    }

    public async Task<Student?> UpdateAsync(Student housingTask, CancellationToken cancellationToken = default)
    {
        bool isContains = await _dbContext.Students.ContainsAsync(housingTask, cancellationToken);
        if (!isContains)
        {
            return null;
        }

        _dbContext.Students.Update(housingTask);
